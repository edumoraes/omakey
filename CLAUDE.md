...
| Lua's `load` honours an explicit `mode = "b"`, and a binary chunk ignores the `_ENV` a sandbox forces | `scoped_load` discards the caller's mode instead of defaulting it. A malformed chunk aborted the VM on every run before that — heap corruption is not something an environment table can contain |
| A stdlib member the sandbox forgets does not cost one binding, it costs all of them | The error reaches the top-level `pcall` and the scan exits. `os.setlocale`, `io.input` and `io.stdin` are there for that reason; `io.output` answers with the sink, because it is the one that opens a file for writing |
